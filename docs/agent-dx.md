...
| Raw payload input | 0 | 1 | 2 | 25 typed operations through `apply --stdin` cover every non-interactive mutation; each shares one core with its flag command; interactive commands are excluded by design and the error says so |
...
`tickets create` wizard (title, Project picker, `$EDITOR`) is TTY-only and is
not an apply operation. Agents pass DESCRIPTION or `--stdin`. `--project`
