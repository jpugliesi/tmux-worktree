...
func (s *Service) planRemoval(reference, currentPane string, opts RemovalOptions) (RemovalPlan, domain.Workspace, []string, error) {
	p, err := s.store.Find(reference)
	if err != nil {
		return RemovalPlan{}, p, nil, err
	}
	plan := RemovalPlan{WorkspaceID: p.ID, WorkspaceName: p.Name, TmuxSession: p.TmuxSession, Blockers: []RemovalBlocker{}}
	if p.ArchivedAt != nil {
	...
	if p.Adopted {
		// twt did not create the directories of an adopted Workspace. Removal
	...
	if p.Status != domain.WorkspaceArchived && p.Status != domain.WorkspaceRemoving && p.Status != domain.WorkspaceSetupFailed {
		plan.Blockers = append(plan.Blockers, RemovalBlocker{
			...
			Message: fmt.Sprintf("Workspace %q is not archived", p.Name),
			Hint:    fmt.Sprintf("Run 'twt workspaces archive %s' before removal.", p.ID),
		})
	}

	// An adopted Workspace has no twt-owned layout to validate: its state is
	...
	allowEmptySnapshot := p.Status == domain.WorkspaceRemoving || p.Status == domain.WorkspaceSetupFailed
	snapshotExists, snapshotErr := s.snapshots.ValidateWorkspace(p.ID, allowEmptySnapshot)
	if snapshotErr != nil {
	...
	if snapshotExists {
		snapshotDirectory, err := s.snapshots.WorkspaceDir(p.ID)
		if err != nil {
			return plan, p, nil, err
		}
	...
	actions = append(actions, RemovalAction{Kind: "delete_workspace_state", Target: p.ID})
	...
	if len(sessions) > 1 {
		plan.Blockers = append(plan.Blockers, RemovalBlocker{
			...
			Message: fmt.Sprintf("Workspace %q owns more than one tmux session", p.Name),
		})
	}
		...
		if err := s.withCacheLock(repository.CachePath, func() error {
			...
			if unknown {
				...
				repositoryBlockers = append(repositoryBlockers, RemovalBlocker{
					...
					Hint:    fmt.Sprintf("Connect to the remote and run the command again, or run 'twt workspaces remove %s --force --apply'.", reference),
				})
				return nil
			}
			repositoryBlockers = append(repositoryBlockers, RemovalBlocker{
...
func (s *Service) Remove(reference, currentPane string, opts RemovalOptions) (RemovalPlan, error) {
	...
	for _, sessionID := range sessions {
		if p.Adopted {
			...
			// running.
			if err := run("", "tmux", s.tmuxArgs("set-option", "-t", sessionID, "-u", workspaceTmuxOption)...); err != nil {
				return plan, fmt.Errorf("release Workspace tmux session: %w", err)
			}
			// Ignore an absent version 1 option. It is removed when it exists,
			// so this session does not stay owned under its old name.
			_ = run("", "tmux", s.tmuxArgs("set-option", "-t", sessionID, "-u", legacyProjectTmuxOption)...)
			continue
		}
		if err := run("", "tmux", s.tmuxArgs("kill-session", "-t", sessionID)...); err != nil {
			return plan, fmt.Errorf("stop Workspace tmux session: %w", err)
		}
	}
	if p.Adopted {
		...
		if err := s.withCacheLock(repository.CachePath, func() error {
			...
			if exists {
				if err := run(repository.CachePath, "git", "branch", "-D", repository.Branch); err != nil {
					return fmt.Errorf("remove Workspace branch %q: %w", repository.Branch, err)
				}
			}
			return nil
		}); err != nil {
...
// Snapshots, Agent Sessions, Prepared Environment record, and the Workspace
// record itself. It touches no worktree or user directory.
func (s *Service) removeState(p domain.Workspace) error {
	if err := s.snapshots.DeleteWorkspace(p.ID, true); err != nil {
		return err
	}
	if err := store.NewAgentStore(s.options.StateDir).DeleteWorkspace(p.ID); err != nil {
		return err
	}
	if p.EnvironmentID != "" {
...
// BulkRemovalPlans returns one removal plan for each archived Workspace whose
// archive time is at least olderThan in the past. An olderThan of zero
// selects all archived Workspaces. The oldest archive comes first.
func (s *Service) BulkRemovalPlans(olderThan time.Duration, opts RemovalOptions) ([]RemovalPlan, error) {
	...
	for _, p := range workspaces {
		if p.Status != domain.WorkspaceArchived || p.ArchivedAt == nil {
			continue
		}
	...
	sort.SliceStable(plans, func(i, j int) bool {
		...
		return plans[i].WorkspaceName < plans[j].WorkspaceName
	})
	return plans, nil
}

// CancelRemoval returns a Workspace from status "removing" to "archived".
func (s *Service) CancelRemoval(reference string) (domain.Workspace, error) {
	lock, err := store.AcquireMutationLock(s.options.StateDir)
	if err != nil {
	...
	if p.Status != domain.WorkspaceRemoving {
		return p, clierr.New(clierr.PreconditionFailed, "Workspace %q has status %q; cancel requires status %q", p.Name, p.Status, domain.WorkspaceRemoving)
	}
	now := s.now()
...
func removalRefusal(workspaceName string, blockers []RemovalBlocker) error {
	...
	refusal := clierr.New(code, "Workspace %q removal is blocked: %s", workspaceName, strings.Join(messages, "; "))
	if hint != "" {
...
// validateRemovalState checks the recorded Workspace state against the layout
// twt owns. Policy refusals return as blockers; the error return is only
// for infrastructure failures.
func (s *Service) validateRemovalState(p domain.Workspace) ([]RemovalBlocker, error) {
	...
	if p.EnvironmentID != "" {
		...
		if errors.Is(err, os.ErrNotExist) && p.Status == domain.WorkspaceRemoving {
	...
	if filepath.Clean(p.Root) != filepath.Clean(expectedRoot) {
		return blocked(BlockerInvalidState, "Workspace %q has an invalid root path", p.Name), nil
	}
	...
	for _, repository := range p.Repositories {
		...
		if repository.Path != filepath.Join(p.Root, repository.Name) {
			return blocked(BlockerInvalidState, "repository %q has a checkout path outside its Workspace root", repository.Name), nil
		}
		...
		if repository.Branch == "" {
			return blocked(BlockerProtectedBranch, "repository %q has no recorded Workspace branch", repository.Name), nil
		}
		protected := spec.DefaultBranch
		if cacheExists {
	...
	for _, entry := range entries {
		if !expectedEntries[entry.Name()] {
			return []RemovalBlocker{{
				...
				Message: fmt.Sprintf("Workspace root %q contains unexpected item %q; move it before removal", p.Root, entry.Name()),
	...
	if markerPresent {
		if err := ValidateWorkspaceMarker(p.Root, p.ID); err != nil {
			return blocked(BlockerUnsafeState, "%s", err.Error()), nil
		}
		return nil, nil
	}
	if !tolerantStatus || len(entries) != 0 {
		return blocked(BlockerUnsafeState, "Workspace root %q has no twt ownership marker", p.Root), nil
	}
	return nil, nil
}
