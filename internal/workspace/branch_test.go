...
const branchTestWorkspaceID = "0123abcd-workspace-id"

func TestResolveWorkspaceBranchOrder(t *testing.T) {
	tests := []struct {
		name    string
		pattern string
		opts    CreateOptions
		...
		{"the default is the Workspace name", "", CreateOptions{}, "fix-auth"},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			got, err := service.resolveWorkspaceBranch("fix-auth", branchTestWorkspaceID, template, test.opts)
			if err != nil {
				t.Fatalf("resolveWorkspaceBranch() error = %v", err)
			}
			if got != test.want {
				t.Fatalf("resolveWorkspaceBranch() = %q, want %q", got, test.want)
			}
		})
	}
}

func TestResolveWorkspaceBranchRejectsTheRepositoryDefaultBranch(t *testing.T) {
	tests := []struct {
		name string
		opts CreateOptions
	}{
		{"a Workspace named like the default branch", CreateOptions{}},
		{"the --branch flag names the default branch", CreateOptions{Branch: "main"}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			service, _ := branchTestService(t)
			_, err := service.resolveWorkspaceBranch("main", branchTestWorkspaceID, branchTestTemplate(), test.opts)
			if err == nil || !strings.Contains(err.Error(), "default branch") {
				t.Fatalf("resolveWorkspaceBranch() error = %v, want a default-branch refusal", err)
			}
			if clierr.CodeOf(err) != clierr.InvalidUsage {
				t.Fatalf("resolveWorkspaceBranch() code = %q, want %q", clierr.CodeOf(err), clierr.InvalidUsage)
			}
		})
	}
}

func TestResolveWorkspaceBranchRejectsAnInvalidBranchName(t *testing.T) {
	service, _ := branchTestService(t)
	_, err := service.resolveWorkspaceBranch("fix-auth", branchTestWorkspaceID, branchTestTemplate(), CreateOptions{Branch: "bad name"})
	if err == nil || clierr.CodeOf(err) != clierr.InvalidUsage {
		t.Fatalf("resolveWorkspaceBranch() error = %v, want invalid_usage", err)
	}
}

func TestResolveWorkspaceBranchNamesTheBranchPrefixSource(t *testing.T) {
	service, _ := branchTestService(t)
	_, err := service.resolveWorkspaceBranch("fix-auth", branchTestWorkspaceID, branchTestTemplate(), CreateOptions{BranchPrefix: "bad prefix "})
	if err == nil || !strings.Contains(err.Error(), `branch prefix "bad prefix "`) {
		t.Fatalf("resolveWorkspaceBranch() error = %v, want the branch-prefix cause", err)
	}
	if clierr.CodeOf(err) != clierr.InvalidUsage {
		t.Fatalf("resolveWorkspaceBranch() code = %q, want %q", clierr.CodeOf(err), clierr.InvalidUsage)
	}
	if hint := clierr.HintOf(err); !strings.Contains(hint, "TWT_BRANCH_PREFIX") || !strings.Contains(hint, "branchPrefix") {
		t.Fatalf("resolveWorkspaceBranch() hint = %q, want the prefix sources", hint)
	}
}

func TestResolveWorkspaceBranchFallsBackOnACollision(t *testing.T) {
	...
	got, err := service.resolveWorkspaceBranch("fix-auth", branchTestWorkspaceID, template, CreateOptions{})
	if err != nil {
		t.Fatalf("resolveWorkspaceBranch() error = %v", err)
	}
