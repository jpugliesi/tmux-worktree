...
func ticketImplementationTask(ticket, claimant string) string {
	lines := []string{
		...
		"Follow the Ticket's ## Plan section when one exists. When the Ticket names",
		"a Project, read the Project plan for context:",
		"twt projects plan show PROJECT --output json",
		"Read the repository's AGENTS.md or CLAUDE.md for its build and test",
		"commands before you guess at them.",
		"",
		fmt.Sprintf("You work as the claimant %q of this Ticket. Do not claim other Tickets, and do not change this Ticket through any command other than the commands below.", claimant),
		"",
		"Attach each pull request to the Ticket as soon as the pull request exists, so reviewers see it immediately:",
		fmt.Sprintf("twt tickets pr add %s --as %s --pr URL", ticket, claimant),
		"",
		"Definition of done. Verify each point before you complete the Ticket:",
		"- every acceptance criterion in the Ticket holds",
		"- the repository's own test gate passes",
		...
		"",
		"Do not end your turn until the Ticket is complete or you are waiting on",
		"the human. When one failure survives three genuinely different attempts,",
		"stop and ask, with what you tried and your recommended option.",
		"A resumed session, or a message about your pull request, continues this",
		"same Ticket: re-claim it if you released it, address the feedback, and",
