...
// Prepare returns one ready Prepared Environment for this Project Template
...
func (s *Service) Prepare(templateName string, template domain.Template) (domain.PreparedEnvironment, error) {
	if err := template.Validate(); err != nil {
		return domain.PreparedEnvironment{}, fmt.Errorf("invalid Project Template %q: %w", templateName, err)
	}
	digests, err := store.Digests(template)
	if err != nil {
		return domain.PreparedEnvironment{}, err
