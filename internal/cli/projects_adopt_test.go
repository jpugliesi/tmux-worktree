...
func TestProjectsAdoptTurnsATmuxSessionIntoAProject(t *testing.T) {
	...
	dry := executeWithOptions(t, options, nil, "projects", "adopt", "handmade", "--dry-run", "--output", "json")
	if !strings.Contains(dry, `"status":"valid"`) {
		t.Fatalf("dry-run adopt = %s", dry)
	}
	if projects, err := store.NewProjectStore(options.StateDir).List(); err != nil || len(projects) != 0 {
		t.Fatalf("dry-run adopt saved a Project: %+v (%v)", projects, err)
	}

	adopted := executeWithOptions(t, options, nil, "projects", "adopt", "handmade", "--output", "json")
	if !strings.Contains(adopted, `"status":"applied"`) {
		t.Fatalf("adopt = %s", adopted)
	}
	project, err := store.NewProjectStore(options.StateDir).Find("handmade")
	if err != nil {
		t.Fatal(err)
	}
	if !project.Adopted || project.Name != "handmade" || project.TmuxSession != "handmade" {
		t.Fatalf("adopted Project = %+v", project)
	}
	if len(project.Repositories) != 1 || project.Repositories[0].Path != repository || project.Repositories[0].Name != "app" {
		t.Fatalf("adopted repositories = %+v", project.Repositories)
	}
	if project.Root != repository {
		t.Fatalf("adopted root = %q", project.Root)
	}
	owner := runCommand(t, "", "tmux", "-L", socket, "show-options", "-t", "handmade", "-v", "@twt_project_id")
	if owner != project.ID {
		t.Fatalf("session marker = %q, want %q", owner, project.ID)
	}

	// The session now belongs to a Project: a second adopt refuses.
	_, _, err = executeRaw(t, options, "projects", "adopt", "handmade")
	if err == nil || clierr.CodeOf(err) != clierr.AlreadyExists {
		t.Fatalf("second adopt = %v", err)
	}

	// The adopted Project works with agent-session discovery: a provider
...
`)
	list := executeWithOptions(t, options, nil, "agents", "list", "--project", "handmade", "--output", "json")
	if !strings.Contains(list, `"id":"codex-adopted"`) || !strings.Contains(list, `"status":"discovered"`) {
		t.Fatalf("agents list for the adopted Project = %s", list)
	}

	// Done archives and removes the adopted Project. The plan keeps the
	...
	if !strings.Contains(done, "keep_directory") || strings.Contains(done, "remove_worktree") {
		t.Fatalf("done plan for the adopted Project = %s", done)
	}
	if _, err := store.NewProjectStore(options.StateDir).Find("handmade"); err == nil {
		t.Fatal("done kept the adopted Project record")
	}
...
func TestProjectsAdoptWorksWithoutARepositoryAndDefaultsToThePaneSession(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git is not installed")
	}
	if _, err := exec.LookPath("tmux"); err != nil {
	...
	adopted := executeWithOptions(t, options, nil, "projects", "adopt", "--output", "json")
	if !strings.Contains(adopted, `"status":"applied"`) {
		t.Fatalf("adopt = %s", adopted)
	}
	project, err := store.NewProjectStore(options.StateDir).Find("scratchpad")
	if err != nil {
		t.Fatal(err)
	}
	if !project.Adopted || len(project.Repositories) != 0 || project.Root != plain {
		t.Fatalf("adopted Project = %+v", project)
	}
	var decoded struct {
		Project struct {
	...
	shown := executeWithOptions(t, options, nil, "projects", "show", "scratchpad", "--output", "json")
	if err := json.Unmarshal([]byte(shown), &decoded); err != nil {
		t.Fatal(err)
	}
	if !decoded.Project.Adopted {
		t.Fatalf("projects show = %s", shown)
	}

	// Archive stops the session; removal then deletes only the twt state.
	// The pane directory survives.
	t.Setenv("TMUX_PANE", "")
	executeWithOptions(t, options, nil, "projects", "archive", "scratchpad", "--output", "json")
	removed := executeWithOptions(t, options, nil, "projects", "remove", "scratchpad", "--apply", "--output", "json")
