...
func TestResolveBoardPickPrefersExactNames(t *testing.T) {
	...
	for _, test := range tests {
		got, err := resolveBoardPick(lines, test.input)
		if err != nil {
			t.Fatalf("resolveBoardPick(%q) = %v", test.input, err)
		}
		if got != test.want {
			t.Fatalf("resolveBoardPick(%q) = %q, want %q", test.input, got, test.want)
		}
	}
	if _, err := resolveBoardPick(lines, "9"); err == nil {
		t.Fatal("out-of-range number succeeded")
	}
	if _, err := resolveBoardPick(lines, ""); err == nil {
		t.Fatal("empty pick succeeded")
	}
}

func TestResolveFzfBoardChoicePrefersSelection(t *testing.T) {
	lines := []string{"(none)", "change-monitor"}
	got, err := resolveFzfBoardChoice(lines, "chan", "change-monitor")
	if err != nil || got != "change-monitor" {
		t.Fatalf("matched selection = %q, %v", got, err)
	}
	got, err = resolveFzfBoardChoice(lines, "new-work", "")
	if err != nil || got != "new-work" {
		t.Fatalf("unmatched query = %q, %v", got, err)
	}
	got, err = resolveFzfBoardChoice(lines, "(none)", "(none)")
	if err != nil || got != "(none)" {
