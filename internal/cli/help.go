...
var commandHelp = map[string]helpContent{
	...
	"twt create": {
		long: "Create a Workspace from a saved Workspace Template. This command is the short form of 'twt workspaces create'. The warm path claims prepared worktrees without a fetch. Use --fresh to fetch and refresh a ready Prepared Environment before the claim. A leftover Prepared Environment is cleaned and returned to the ready pool when its source session is gone. An Environment that is still in use is not a create target. Without NAME at a text terminal, twt asks for a Workspace name. Without --template at a text terminal, twt shows a Workspace Template picker when more than one Template exists. A script uses the last-used Template, or it passes --template. A pipe and --output json still require NAME.", example: "  twt create\n  twt create fix-auth --template everysphere\n  twt create fix-auth --template everysphere --fresh",
	},
	"twt next": {
	...
	"twt reset": {
		long: "Kill the process in every pane in the current tmux window and start an interactive shell in that pane's directory. The other panes reset at the same time. The pane that runs this command resets last. Run it from a tmux pane.", example: "  twt reset\n  twt reset --dry-run --output json\n  twt reset --output json",
	},
	"twt sync": {
		long: "Reconcile the Tickets home with its git remote. This command is the short form of 'twt tickets sync'. With --project, also reconcile that Project's dispatch Sessions.", example: "  twt sync --output json\n  twt sync --project core --dry-run --output json\n  twt sync --project core --output json",
	},
	"twt agents": {
	...
	"twt workspaces rename": {
		long: "Change the display name of a Workspace. twt also renames the owned tmux session to match. It writes the new name on the Prepared Environment claim so the old name is free. The Workspace ID, root, checkouts, branches, and Agent Sessions do not change. One NAME argument renames the current Workspace. Two arguments set the Workspace and the new name. Without arguments, twt shows the Workspace picker and asks for the new name. When an archived Workspace holds NAME, an interactive terminal shows its removal plan and asks before twt removes it. Use --remove-archived to remove it without the question.", example: "  twt workspaces rename auth-fix\n  twt workspaces rename fix-auth auth-fix\n  twt workspaces rename fix-auth auth-fix --remove-archived\n  twt workspaces rename",
	},
	"twt workspaces set": {
		long: "Set the Ticket Project on one Workspace. The Project must not be closed. When the Workspace links Tickets, every Ticket must already belong to that Project. twt does not move Tickets, checkouts, or Environments.", example: "  twt workspaces set current --project change-monitor\n  twt workspaces set fix-auth --project change-monitor --dry-run --output json",
	},
	...
	"twt tickets list": {
		long: "List Tickets sorted by the STATUS column, most actionable first, then by priority, then by slug. The list uses --project, then TWT_PROJECT, then the current Workspace Project. With no Project in scope, the list includes every active Project and omits Tickets from paused Projects. A scoped text list is a simple table. --all-projects lists every Project, including paused Projects, even when a Workspace Project is set.\n\nA wide text table has a PROJECT column. The LABELS column lists each Ticket's labels, comma-separated. JSON stays a flat array. The list hides done and wontfix Tickets by default. --all includes them. --ready lists only pickable work: ready-for-agent, unclaimed, and with every blocker done or wontfix. --claimed lists Tickets that have a claimant. --label keeps Tickets that carry that label. Repeat the flag to require every named label. --label does not change Project scope. Pass --all-projects for a cross-Project label feed.\n\nThe STATUS column shows a derived value. A claimed Ticket is in-progress. A claimed Ticket that waits on the human is needs-input. A Ticket with pull requests in review is in-review. Other rows show the stored status. --status matches the STATUS column or a stored status such as ready-for-agent. JSON status stays the stored field. Do not use --status with --ready. --project and --all-projects cannot be used together.", example: "  twt tickets list --ready --output json\n  twt tickets list --status in-progress\n  twt tickets list -A --claimed --output json\n  twt tickets list --label change-monitor -A --output json\n  twt tickets list --project change-monitor --limit 10",
	},
	"twt tickets queue": {
	...
	"twt projects close": {
		long: "Close one Project. With no open Tickets, the command closes it immediately. At a text terminal, twt asks before it sets open Tickets to wontfix. A script must pass --force when open Tickets remain. Close clears each affected Ticket claim and Workspace link. It does not stop Workspaces or agents. It keeps the Project directory, index.md, and plan.md. Close of a paused Project clears the pause. Closed Projects do not appear in default Project lists or completion.", example: "  twt projects close change-monitor\n  twt projects close change-monitor --force --output json",
	},
	"twt projects pause": {
		long: "Pause one Project. The Project stays writable. Default Project lists and completion omit it. --all includes it. New Tickets, plan writes, and Workspace links still work. Pause of a closed Project fails.", example: "  twt projects pause change-monitor\n  twt projects pause change-monitor --dry-run --output json",
	},
	"twt projects resume": {
		long: "Resume one paused Project. The Project returns to the default list. Resume does not open a closed Project.", example: "  twt projects resume change-monitor\n  twt projects resume change-monitor --dry-run --output json",
	},
	"twt projects list": {
		long: "List every active Project with its status and WORK counts. STATUS is active, paused, or closed. WORK is in-progress plus todo Tickets over open Tickets. Open excludes done and wontfix. JSON also has the board counts. Use 'twt projects get' for the board. --all includes paused and closed Projects. The --all list shows active, then paused, then closed.", example: "  twt projects list\n  twt projects list --all --output json",
	},
	"twt projects get": {
