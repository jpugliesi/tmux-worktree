...
var commandHelp = map[string]helpContent{
	...
	"twt projects list": {
		long: "List every active Project with its status and Ticket counts by board state. STATUS is active or closed. The count columns are WAITING, PROGRESS, REVIEW, READY, BLOCKED, TODO, and DONE. TODO holds open Tickets that are not in the other columns, such as needs-triage. TICKETS is the total, including closed Tickets. --all includes closed Projects.", example: "  twt projects list\n  twt projects list --all --output json",
	},
	"twt projects get": {
