...
var commandHelp = map[string]helpContent{
	...
	"twt tickets list": {
		long: "List Tickets sorted by priority, then by slug. A list needs a Project in scope unless --all-projects is set. The order is --project, then TWT_PROJECT, then the current Workspace Project, then a saved current Project. The saved file is only for interactive text. JSON, ndjson, and non-interactive calls never read it. A scoped text list is a simple table.\n\n--all-projects lists every Project. Its text table has a PROJECT column. JSON stays a flat array. The list hides done and wontfix Tickets by default. --all includes them, and an explicit --status shows that status. --ready lists only pickable work: ready-for-agent, unclaimed, and with every blocker done or wontfix. --claimed lists Tickets that have a claimant.\n\n--status is a raw filter. Do not use it together with --ready. --project and --all-projects cannot be used together.", example: "  twt tickets list --project change-monitor --ready --output json\n  twt tickets list --all-projects --claimed --output json\n  twt tickets list --project change-monitor --limit 10",
	},
	"twt tickets use": {
		long: "Save the current Project for interactive text ticket commands. The save lives in the state directory. Agents must not run this command. They pass --project or set TWT_PROJECT. With no PROJECT, twt prints the saved name. --unset clears the save.", example: "  twt tickets use change-monitor\n  twt tickets use\n  twt tickets use --unset",
	},
	"twt tickets queue": {
		long: "Show the complete open Ticket dependency graph for one Project. The Project comes from --project, then TWT_PROJECT, then the current Workspace Project, then a saved current Project. The saved file is only for interactive text. ready contains only ready-for-agent, unclaimed Tickets whose blockers are done or wontfix. The command sorts graph and ready by priority and then by Ticket slug. --limit cuts ready but does not cut graph.\n\ncycles reports dependency cycles that stop affected Tickets from becoming ready.", example: "  twt tickets queue --project change-monitor --output json\n  twt tickets queue --project change-monitor --limit 4 --fields ready,readyTotalCount --output json",
	},
	"twt tickets show": {
