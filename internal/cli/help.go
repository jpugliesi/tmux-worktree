...
var commandHelp = map[string]helpContent{
	...
	"twt projects list": {
		long: "List every active Project with its status and WORK counts. STATUS is active or closed. WORK is in-progress plus todo Tickets over open Tickets. Open excludes done and wontfix. JSON also has the board counts. Use 'twt projects get' for the board. --all includes closed Projects.", example: "  twt projects list\n  twt projects list --all --output json",
	},
	"twt projects get": {
