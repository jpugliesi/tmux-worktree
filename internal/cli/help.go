...
var commandHelp = map[string]helpContent{
	...
	"twt tickets plan": {
		long: "Replace the ## Plan section of one Ticket. twt keeps every other section. The argument - reads the plan from standard input. In an interactive terminal without -, twt opens VISUAL or EDITOR on a draft of the current plan and writes the saved result. twt tickets plan edit TICKET is the same write. Planning agents write their decision-complete plan here before implementation. A claimed Ticket requires the matching --as claimant.", example: "  twt tickets plan fix-auth\n  twt tickets plan edit fix-auth\n  printf '%s' \"$PLAN\" | twt tickets plan fix-auth - --as codex-fix-auth --output json\n  printf '%s' \"$PLAN\" | twt tickets plan fix-auth - --dry-run --output json",
	},
	"twt tickets plan edit": {
		long: "Replace the ## Plan section of one Ticket. This is the same write as twt tickets plan TICKET. The argument - reads the plan from standard input. In an interactive terminal without -, twt opens VISUAL or EDITOR on a draft of the current plan and writes the saved result. A claimed Ticket requires the matching --as claimant.", example: "  twt tickets plan edit fix-auth\n  printf '%s' \"$PLAN\" | twt tickets plan edit fix-auth - --as codex-fix-auth --output json",
	},
