...
// discoverWorkspaceSessions finds the provider sessions of the Workspace that no
...
func discoverWorkspaceSessions(workspace domain.Workspace, stateDir string, linked []domain.AgentSession) ([]transcriptservice.DiscoveredSession, error) {
	return discoverWorkspaceSessionsSince(workspace, stateDir, linked, time.Time{})
}

// discoverWorkspaceSessionsSince finds the provider sessions of the Workspace that
// changed after one time. Shell completion uses a window, so a key press reads
// only the recent provider files.
func discoverWorkspaceSessionsSince(workspace domain.Workspace, stateDir string, linked []domain.AgentSession, since time.Time) ([]transcriptservice.DiscoveredSession, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return nil, fmt.Errorf("find home directory: %w", err)
	}
	return transcriptservice.New(home, stateDir).Discover(workspace, transcriptservice.DiscoverOptions{Linked: linked, Since: since})
}
...
func adoptDiscoveredSession(agents *agentservice.Service, workspace domain.Workspace, session transcriptservice.DiscoveredSession) (domain.AgentSession, error) {
	...
	return agents.Register(workspace, session.Provider, "", "", session.SessionID, resumeCommand)
}

// findOrAdoptAgent resolves one AGENT reference for a Workspace. A registered
// Agent Session always wins, also when the reference is a prefix. When the
// registered lookup misses, twt scans the provider stores of the Workspace: a
...
func findOrAdoptAgent(command *cobra.Command, agents *agentservice.Service, workspace domain.Workspace, stateDir, reference string) (domain.AgentSession, bool, error) {
...
func matchDiscoveredSession(agents *agentservice.Service, workspace domain.Workspace, stateDir, reference string) (transcriptservice.DiscoveredSession, bool, error) {
	registered, err := agents.List(workspace.ID)
	if err != nil {
		return transcriptservice.DiscoveredSession{}, false, err
	}
	sessions, err := discoverWorkspaceSessions(workspace, stateDir, registered)
	if err != nil {
		return transcriptservice.DiscoveredSession{}, false, err
	}
...
// nothing.
func validateAdoption(agents *agentservice.Service, workspace domain.Workspace, session transcriptservice.DiscoveredSession) (domain.AgentSession, error) {
	resumeCommand := transcriptservice.ResumeCommand(session.Provider, session.SessionID)
	if len(resumeCommand) == 0 {
		return domain.AgentSession{}, clierr.New(clierr.PreconditionFailed, "provider %q gives no resume command", session.Provider)
	}
	if err := agents.ValidateRegistration(workspace, session.Provider, "", session.SessionID, resumeCommand); err != nil {
		return domain.AgentSession{}, err
	}
	existing, err := agents.List(workspace.ID)
	if err != nil {
		return domain.AgentSession{}, err
	}
	return agentservice.BuildSession(workspace, session.Provider, "", "", session.SessionID, resumeCommand, existing, time.Now().UTC())
}
...
func notRegisteredForRemoval(agents *agentservice.Service, workspace domain.Workspace, stateDir, reference string, err error) error {
	if err == nil || clierr.CodeOf(err) != clierr.NotFound {
		return err
	}
	if _, found, matchErr := matchDiscoveredSession(agents, workspace, stateDir, reference); matchErr == nil && found {
